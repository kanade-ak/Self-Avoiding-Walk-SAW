...
:build
cl.exe /nologo /O2 /GL /EHsc /std:c++20 /utf-8 /W4 /permissive- /DNDEBUG moto_probe_mph_inplace.cpp /Fe:moto_probe_mph_inplace.exe /link /LTCG
if errorlevel 1 goto :error
del /q moto_probe_mph_inplace.obj >nul 2>&1

cl.exe /nologo /O2 /EHsc /std:c++20 /utf-8 /W4 /permissive- /DNDEBUG /openmp moto_probe_mph_inplace.cpp /Fe:moto_probe_mph_inplace_parallel.exe
if errorlevel 1 goto :error
del /q moto_probe_mph_inplace.obj >nul 2>&1

echo Built: %CD%\moto_probe_mph_inplace.exe
echo Built: %CD%\moto_probe_mph_inplace_parallel.exe
popd
exit /b 0

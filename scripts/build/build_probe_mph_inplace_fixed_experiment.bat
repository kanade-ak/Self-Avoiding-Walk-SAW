...
:build
cl.exe /nologo /O2 /GL /EHsc /std:c++20 /utf-8 /W4 /permissive- /DNDEBUG src\experiments\moto_probe_mph_inplace_fixed.cpp /Fe:build\moto_probe_mph_inplace_fixed.exe /Fo:build\moto_probe_mph_inplace_fixed.obj /link /LTCG
if errorlevel 1 goto :error
del /q build\moto_probe_mph_inplace_fixed.obj >nul 2>&1

cl.exe /nologo /O2 /EHsc /std:c++20 /utf-8 /W4 /permissive- /DNDEBUG /openmp src\experiments\moto_probe_mph_inplace_fixed.cpp /Fe:build\moto_probe_mph_inplace_fixed_parallel.exe /Fo:build\moto_probe_mph_inplace_fixed.obj
if errorlevel 1 goto :error
del /q build\moto_probe_mph_inplace_fixed.obj >nul 2>&1

echo Built: %CD%\build\moto_probe_mph_inplace_fixed.exe
echo Built: %CD%\build\moto_probe_mph_inplace_fixed_parallel.exe
popd
exit /b 0

...
private:
    void prepare_count_width_for_next_update() {
        ...
        while (Count::active_limb_count() < required_limb_count) {
            // 全limbは初期化時にゼロ化済み。未使用limbへは昇格まで
            // 一切書き込まないため、配列全走査なしで安全に有効化できる。
            Count::add_limb();
        }
    }
...
#pragma omp parallel for schedule(static)
        ...
        for (std::int64_t code = 0;
            ...
            value[static_cast<Code>(code)].clear_all_limbs();
        }
...
#pragma omp parallel for schedule(static)
        ...
        for (std::int64_t code = 0;
            ...
            deferred[static_cast<Code>(code)].clear_all_limbs();
        }

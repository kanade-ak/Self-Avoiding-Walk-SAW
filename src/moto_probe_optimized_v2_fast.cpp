...
public:
    ...
    void clear() {
        ...
        if (generation_ == 0) {
            for (const auto& page : pages_) {
                if (page) {
                    std::fill(page->stamps.begin(), page->stamps.end(),
                              std::uint8_t{0});
    ...
    void add(StateKey key, const Count& ways) {
        ...
        if (!pages_[page_number]) {
            pages_[page_number] =
                std::make_unique_for_overwrite<Page>();
        }
    ...
    std::size_t page_storage_bytes() const noexcept {
        return allocated_pages() * sizeof(Page);
    }
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        std::cout
            << "algorithm = packed-rank fast-page DP (exact 384-bit)\n"
            << "n = " << n << '\n'
